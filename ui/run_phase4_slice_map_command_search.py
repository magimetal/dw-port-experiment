...
def _run_search_flow(session: MainLoopSession) -> tuple[str, str, str, bool, bool]:
    ...
    return (
        ...
        "THOU HAST FOUND A TREASURE CHEST." in result.frame,
    )
...
def main() -> int:
    ...
    vectors = {
        ...
        "search_chest": {
            ...
            "screen_mode": chest[2],
            "frame_contains_chest": chest[4],
        },
    }
    (fixtures_dir / "main_loop_map_command_search_vectors.json").write_text(
    ...
    checks = {
        ...
        "search_chest_tile_detects_extracted_chest": (
            vectors["search_chest"]["action"] == "map_search"
            and vectors["search_chest"]["action_detail"] == "chest:index:0;contents:19"
            and vectors["search_chest"]["screen_mode"] == "dialog"
            and vectors["search_chest"]["frame_contains_chest"] is True
        ),
    }

    artifact = {
        "slice": "phase4-main-loop-map-command-search",
        "all_passed": all(checks.values()),
        "checks": checks,
        "outputs": {
            "report": "artifacts/phase4_main_loop_map_command_search.json",
            "vectors_fixture": "tests/fixtures/main_loop_map_command_search_vectors.json",
        },
        "rom": {
        ...
        "scope_note": (
            "Phase 4 bounded SEARCH integration: map root SEARCH now resolves deterministic no-result dialog "
            "and deterministic chest-tile detection using extracted chest data."
        ),
    }
    (artifacts_dir / "phase4_main_loop_map_command_search.json").write_text(
